...
/*
	...
	public @ResponseBody ResponseEntity<List<Specialization>> readById(
		...
		try {
			List<Specialization> specialization = specializationService
					.readById(id);
			return new ResponseEntity<List<Specialization>>(specialization,
					HttpStatus.OK);
		} catch (Exception e) {
			e.printStackTrace();
			return new ResponseEntity<List<Specialization>>(
