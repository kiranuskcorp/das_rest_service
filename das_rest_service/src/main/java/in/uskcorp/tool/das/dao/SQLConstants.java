...
public class SQLConstants {
	...
	public static final String DOCTOR_SELECT = "SELECT d.*,GROUP_CONCAT(sp.name), dp.department_name as department_name,hp.name as hospital_name  FROM user d, specialization sp,  hospital hp, department dp where dp.id= d.department_id and hp.id= d.hospital_id and d.role_id=8 and d.active_flag=0 and FIND_IN_SET(sp.id, d.specialization_id) GROUP BY d.specialization_id ORDER by id desc";
