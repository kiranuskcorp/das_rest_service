...
public class ReportingController extends APIController<Reporting> {
	...
	public @ResponseBody ResponseEntity<List<Reporting>> getDetails(@RequestParam Integer id, @RequestParam String from,
			@RequestParam String to) {
		try {
			List<Reporting> reporting = ((ReportingService) getService()).getDetails(id, from, to);
			return new ResponseEntity<List<Reporting>>(reporting, HttpStatus.OK);
		} catch (Exception e) {
			e.printStackTrace();
			return new ResponseEntity<List<Reporting>>(HttpStatus.SERVICE_UNAVAILABLE);
		}
	}
}
